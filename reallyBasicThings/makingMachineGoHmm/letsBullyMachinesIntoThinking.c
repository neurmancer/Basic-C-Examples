...
int main(void)
{
    ...
    float w = randFloat()*5.0f;
    float dCost = (cost(w+eps)-cost(w))/eps;    //You 'member the definition of derivative right? That's kinda it lol you can call it just slope, finite differance etc.
    w -= dCost*rate;
...
{   
    ...
    {
        float x = train[i][0];
        float y = x*w;  //w is the tuning paramter type shit dunno yet...
